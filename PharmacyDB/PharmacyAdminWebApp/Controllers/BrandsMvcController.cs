...
namespace PharmacyAdminWebApp.Controllers
{
    public class BrandsMvcController : BaseController
    {
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync("/Brands/Search?value=" + value);
            if (response.IsSuccessStatusCode)
            {
                var responseStream = await response.Content.ReadAsStreamAsync();
                var options = new JsonSerializerOptions
        ...
        public async Task<IActionResult> Edit(Brand _brand)
        {
              Brand brand = new Brand
                ...
                HttpResponseMessage response = await _httpClient.PutAsync("/Brands", content);
                if (response.IsSuccessStatusCode)
                {
                    var responseStream = await response.Content.ReadAsStreamAsync();
                    var options = new JsonSerializerOptions
        ...
        public async Task<IActionResult> DeleteBrandMvc(int id)
        {
            ...
            HttpResponseMessage response = await _httpClient.DeleteAsync($"/Brands?brandId={id}");
            if (response.IsSuccessStatusCode)
            {
                var responseStream = await response.Content.ReadAsStreamAsync();
                var options = new JsonSerializerOptions
