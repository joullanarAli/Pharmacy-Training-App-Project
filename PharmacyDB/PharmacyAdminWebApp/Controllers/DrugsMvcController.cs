...
namespace PharmacyAdminWebApp.Controllers
{
    public class DrugsMvcController : BaseController
    {
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync("/Drugs");
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync("/Drugs/Search?value=" + value);
            if (response.IsSuccessStatusCode)
            {
                var responseStream = await response.Content.ReadAsStreamAsync();
                var options = new JsonSerializerOptions
        ...
        public async Task<IActionResult> Create(DrugViewModel drugVm)
        {
            ...
            else
            {
                ...
                drugViewModel.ActiveIngredientNames = activeIngredientList;
                TempData["Message"] = "please choose/rechoose the forms";
                return View(drugViewModel);
            }
     ...
     */
        ...
        {
            HttpResponseMessage response = await _httpClient.DeleteAsync($"/Drugs?drugId={id}");
            if (response.IsSuccessStatusCode)
            {
                var responseStream = await response.Content.ReadAsStreamAsync();
                var options = new JsonSerializerOptions
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync("/Drugs/GetDrugForms?drugId=" + drugId);
            var httpClient = _httpClientFactory.CreateClient("Drugs");
            if (response.IsSuccessStatusCode)
            {
                var responseStream = await response.Content.ReadAsStreamAsync();
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync("/Drugs/GetDrugActiveIngredients?drugId=" + drugId);
            var httpClient = _httpClientFactory.CreateClient("Drugs");
            if (response.IsSuccessStatusCode)
            {
                var responseStream = await response.Content.ReadAsStreamAsync();
