...
namespace PharmacyAdminWebApp.Controllers
{
    public class FormsMvcController : BaseController
    {
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync("/Forms");
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync("/Forms/Search?value=" + value);
            if (response.IsSuccessStatusCode)
            {
                var responseStream = await response.Content.ReadAsStreamAsync();
                var options = new JsonSerializerOptions
        ...
        public async Task<IActionResult> Edit(Form _form)
        {
            ...
            HttpResponseMessage response = await _httpClient.PutAsync("/Forms", content);
            if (response.IsSuccessStatusCode)
            {
                var responseStream = await response.Content.ReadAsStreamAsync();
                var options = new JsonSerializerOptions
        ...
        {
            Form form = (await _unitOfWork._formRepository.GetById(id));
            HttpResponseMessage response = await _httpClient.DeleteAsync($"/Forms?formId={id}");
            if (response.IsSuccessStatusCode)
            {
                var responseStream = await response.Content.ReadAsStreamAsync();
                var options = new JsonSerializerOptions
