...
namespace PharmacyAdminWebApp.Controllers
{
    public class CategoriesMvcController : BaseController
    {
        ...
        {
            HttpResponseMessage response = await _httpClient.GetAsync("/Categories/Search?value=" + value);
            if (response.IsSuccessStatusCode)
            {
                var responseStream = await response.Content.ReadAsStreamAsync();
                var options = new JsonSerializerOptions
        ...
        public async Task<IActionResult> Edit(Category _category)
        {
            ...
            HttpResponseMessage response = await _httpClient.PutAsync("/Categories", content);
            if (response.IsSuccessStatusCode)
            {
                var responseStream = await response.Content.ReadAsStreamAsync();
                var options = new JsonSerializerOptions
        ...
        {
            Category category = (await _unitOfWork._categoryRepository.GetById(id));
            HttpResponseMessage response = await _httpClient.DeleteAsync($"/Categories?categoryId={id}");
            if (response.IsSuccessStatusCode)
            {
                var responseStream = await response.Content.ReadAsStreamAsync();
                var options = new JsonSerializerOptions
